...
class SparseObsDataset(Dataset):
    ...
    def process(self):
        for raw_file_path in tqdm(self.raw_paths, desc="Processing"):
            ...
            for sparsity in self.sparsities:
                train_index, test_index = self._split_indices(n_samples=len(x),test_size=sparsity)
                (
                    edge_index,
                    dists,
                    cdist,
...
class RandomLeaveOneOutDataset(Dataset):
    def __init__(
        self,
        root,
        processed,
    ...
    def processed_file_names(self):
        return [
            f"{raw_file_name.split('.')[0]}_{self.location}_{time}.pt"
            for raw_file_name in self.raw_file_names
            for time in range(1,self.n_times+1)
        ]
    ...
    def process(self):
        for raw_file_path in tqdm(self.raw_paths, desc="Processing"):
            df = read_new_zealand_data(raw_file_path, self.location)
            x, y = self._get_x_y(df, feature_cols=["lat", "lon", "elev"], obs_col="obs")
            for time in range(1,1+self.n_times):
                train_index, test_index = self._split_indices(n_samples=len(x),test_size=1)
                (
                    edge_index,
                    dists,
                    cdist,
                ...
                torch.save(
                    ...
                    os.path.join(
                        self.processed_dir,
                        f"{os.path.basename(raw_file_path).split('.')[0]}_{self.location}_{time}.pt",
                    ),
                )

    def _get_x_y(
        self, df: pd.DataFrame, feature_cols=["lat", "lon", "elev","CMORPH"], obs_col="obs"
    ):
        """
        将数据集拆分为特征和标签
        """
...
if __name__ == "__main__":
    dataset = RandomLeaveOneOutDataset("dataset", "random_south", "south")
    print(dataset)
